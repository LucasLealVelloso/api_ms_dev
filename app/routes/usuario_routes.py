...
from db.models import Usuarios as UsuarioModel
...
def create(request: UsuarioRequest, db: Session = Depends(get_db)):
    usuario = UsuarioRepository.save(db, UsuarioModel(**request.dict()))
...
def update(id: int, request: UsuarioRequest, db: Session = Depends(get_db)):
    ...
    usuario = UsuarioRepository.save(db, UsuarioModel(id=id, **request.dict()))
    return UsuarioResponse.from_orm(usuario)